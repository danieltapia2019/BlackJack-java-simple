...
//            mazo[i].mostrarCarta(); //linea para verificar que el mazo se creo en orden
    ...
    public static void mezclaMazoAux(Carta[] mazoaux){ 
        int m=(int) (Math.random() * (4 - 1 + 1) + 1);
        switch(m){
            case 1:
                mezclarForma1(mazoaux);
                mezclarForma2(mazoaux);
                break;
            case 2:
                mezclarForma2(mazoaux);
                mezclarForma1(mazoaux);
    ...
    public static void mezclarForma1( Carta[] mazoaux) { //intercalar sobrecargado
        int veces = (int) (Math.random() * (4 - 3 + 1) + 3); //El croupier va hacer de 3 a 4 veces este tipo de mezcla de cartas
        for (int v = 0; v < veces; v++) {

            int k = (int) (Math.random() * (20 - 15 + 1) + 15);  //Cortara de 15 a 20 cartas ,es al azar
            ...
            for (int i = 0; i < mazoaux.length; i++) {
                ...
                if (i < k) {
                    aux[i] = mazoaux[i];
                    mazoaux[i] = mazoaux[i + k];
                } else if (i < mazoaux.length - k) {
                    mazoaux[i] = mazoaux[i + k];
                } else if (i < mazoaux.length - (k - div)) { //cuando entra aca se suelta una parte de las cartas seleccionadas

                    mazoaux[i] = aux[indice];

                    indice++;
                } else {
                    if (i == mazoaux.length - (k - div)) { // cuando entra aca termina de soltar las cartas restantes de las seleccinoadas
                        indice = 0;
                    }
...
//                    System.out.println("Aux2");con esta linea va viendo si entro carta de esta parte.Es solo para chequaear
    ...
    public static void mezclarForma2(Carta[] mazoaux) {//separar el mazo en dos partes por igual aproximadamente y ir soltando cartas de ambas partes
        int veces = (int) (Math.random() * (4 - 3 + 1) + 3); //El croupier va hacer de 3 a 4 veces este tipo de mezcla de cartas
        ...
        for (int v = 0; v < veces; v++) {
            ...
            Carta[] aux1 = new Carta[k];
            Carta[] aux2 = new Carta[mazoaux.length - k]; //auxiliares para almacenar las dos partes del mazo
            int indice1 = 0;
            int indice2 = 0;
